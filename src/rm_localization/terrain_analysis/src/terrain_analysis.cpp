...
{
  // 声明参数
  declare_parameter<std::string>("sensor_frame", sensor_frame_); // 传感器坐标系名称
  declare_parameter<double>("scan_voxel_size", scan_voxel_size_); // 点云下采样时体素的大小(m)
  declare_parameter<double>("decay_time", decay_time_); // 点云保留时间(s)
  declare_parameter<double>("no_decay_dis", no_decay_dis_); // 车辆点云有效检测距离(m)
  declare_parameter<double>("clearing_dis", clearing_dis_); // 点云清除距离阈值(m)
  declare_parameter<bool>("use_sorting", use_sorting_); // 是否使用排序处理点云
  declare_parameter<double>("quantile_z", quantile_z_); // 地面高度分位数阈值(0-1)
  declare_parameter<bool>("consider_drop", consider_drop_); // 是否考虑斜坡作为障碍物
  declare_parameter<bool>("limit_ground_lift", limit_ground_lift_); // 是否限制地面抬升
  declare_parameter<double>("max_ground_lift", max_ground_lift_); // 最大地面抬升高度(m)
  declare_parameter<bool>("clear_dy_obs", clear_dy_obs_); // 是否清除动态障碍物
  declare_parameter<double>("min_dy_obs_dis", min_dy_obs_dis_); // 动态障碍物最小检测距离(m)
  declare_parameter<double>("min_dy_obs_angle", min_dy_obs_angle_); // 动态障碍物最小检测角度(度)
  declare_parameter<double>("min_dy_obs_rel_z", min_dy_obs_rel_z_); // 相对车体的地平面高度(m)
  declare_parameter<double>("abs_dy_obs_rel_z_thre", abs_dy_obs_rel_z_thre_); // 动态障碍物Z轴绝对阈值(m)
  declare_parameter<double>("min_dy_obs_vfov", min_dy_obs_vfov_); // 垂直视场最小角度(度)
  declare_parameter<double>("max_dy_obs_vfov", max_dy_obs_vfov_); // 垂直视场最大角度(度)
  declare_parameter<int>("min_dy_obs_point_num", min_dy_obs_point_num_); // 动态障碍物最小点数
  declare_parameter<bool>("no_data_obstacle", no_data_obstacle_); // 是否将无数据区域视为障碍
  declare_parameter<int>("no_data_block_skip_num", no_data_block_skip_num_); // 无数据区域评估次数
  declare_parameter<int>("min_block_point_num", min_block_point_num_); // 区块最小有效点数
  declare_parameter<double>("vehicle_height", vehicle_height_); // 车辆高度(m)
  declare_parameter<int>("voxel_point_update_thre", voxel_point_update_thre_); // 体素更新点数阈值
  declare_parameter<double>("voxel_time_update_thre", voxel_time_update_thre_); // 体素更新时间阈值(s)
  declare_parameter<double>("min_rel_z", min_rel_z_); // 点云裁剪最小相对高度(m)
  declare_parameter<double>("max_rel_z", max_rel_z_); // 点云裁剪最大相对高度(m)
  declare_parameter<double>("dis_ratio_z", dis_ratio_z_); // 点云高度与距离比例
  declare_parameter<double>("clear_interval", clearing_interval_); // 清除点云周期(s)
  ...
  get_parameter("clear_interval", clearing_interval_);
  ...
  // 设置清除定时器
  clearing_timer_ = create_wall_timer(
    std::chrono::duration<double>(clearing_interval_),
    std::bind(&TerrainAnalysisNode::clearingTimerCallback, this));
}

void TerrainAnalysisNode::clearingTimerCallback()
{
  no_data_inited_ = 0;
